...
###############################################################
# Mode(s) pour les données continues
###############################################################
...
def Mode_C(data,k):
    ...
    i_max=[i for i in range(k) if NN[i]==np.max(NN)]
    return [round(Ext[i_max[0]],5)]+[round(Ext[i_max[0]+1],5)]
...
def histo_discretes(data,nom=None):
    ...
    if nom is None:
        ...
        plt.savefig("histo.png", format="png")
        plt.close()
...
def Lecture_data(Nom_fichier):
    ...
    data=[line.strip().split() for line in data_temp]
    ...
    colonne=len(data[0]) # data[0] : première ligne
